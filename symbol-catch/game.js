...
const symbolResultCard = document.querySelector("#symbol-result-card");
const symbolResultCardScore = document.querySelector("#symbol-result-card-score");
const symbolResultCardDetail = document.querySelector("#symbol-result-card-detail");
const symbolResultCardNote = document.querySelector("#symbol-result-card-note");
...
function refreshResultCard() { if (!symbolResultCard || !symbolResultCardScore || !symbolResultCardDetail || !symbolResultCardNote) return; if (!lastRun) { symbolResultCard.hidden = true; return; } symbolResultCard.hidden = false; symbolResultCardScore.textContent = Number(lastRun.score || 0).toLocaleString(); symbolResultCardDetail.textContent = `Streak ${Number(lastRun.bestStreak || 0)} // ${Number(lastRun.runSeconds || 0)} seconds`; symbolResultCardNote.textContent = lastRun.score > bestScore() ? "New best score on this device." : "Try to beat your best run."; }
function start() { reset(); running = true; message.hidden = true; startButton.textContent = "Restart challenge \u2192"; status.textContent = "Catch the glowing target symbol. Perks unlock every 8 correct catches."; announceTarget(); if (symbolResultCard) symbolResultCard.hidden = true; canvas.focus(); }
...
canvas.addEventListener("pointerdown", (event) => { event.preventDefault(); setPointer(event); });
startButton.addEventListener("click", start);
document.querySelectorAll("[data-move]").forEach((button) => { const side = button.dataset.move; button.addEventListener("pointerdown", (event) => { event.preventDefault(); keys[side] = true; }); button.addEventListener("pointerup", () => keys[side] = false); button.addEventListener("pointerleave", () => keys[side] = false); button.addEventListener("pointercancel", () => keys[side] = false); });
...
reset(); refreshResultCard(); status.textContent = bestScore() ? `Best score on this device: ${bestScore()}.` : "No best score yet. Start a challenge."; requestAnimationFrame(frame);
initSymbolOnline();
