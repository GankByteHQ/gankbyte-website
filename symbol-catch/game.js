...
async function loadSymbolLeaderboard() { if (!symbolLeaderboardBody) return; if (!symbolClient) { symbolLeaderboardBody.innerHTML = '<tr><td colspan="4">Global scores need the XP backend connection.</td></tr>'; return; } const result = await symbolClient.from("symbol_catch_leaderboard").select("display_name,best_score,best_streak").order("best_score", { ascending: false }).limit(25); if (result.error) { symbolLeaderboardBody.innerHTML = '<tr><td colspan="4">Global scores are not available yet.</td></tr>'; return; } symbolLeaderboardBody.innerHTML = result.data?.length ? result.data.map((row, index) => `<tr><td>${index + 1}</td><td>${escapeSymbolHtml(row.display_name || "GankByte Player")}</td><td>${Number(row.best_score || 0).toLocaleString()}</td><td>${Number(row.best_streak || 0)}</td></tr>`).join("") : '<tr><td colspan="4">No scores yet. Be the first to submit.</td></tr>'; }
