...
let banked = 0, risk = 0, multiplier = 1, lives = 3, xp = 0, maxMultiplier = 1;
...
function updateHud() {
  ...
  const close = player && distance(player, BANK) < BANK.radius + player.radius + 12;
  bankButton.disabled = !running || !risk || !close;
  bankButton.textContent = close ? "Bank score  ↑" : "Return to bank terminal";
}
...
function spawnPowerup() { const types = ["magnet", "shield", "freeze", "double", "ghost", "time"]; powerups.push({ x: random(40, WIDTH - 40), y: random(40, HEIGHT - 40), type: types[Math.floor(random(0, types.length))], pulse: 0, expires: elapsed + 12 }); }
function reset() {
  running = false; elapsed = 0; timeLeft = 60; banked = 0; risk = 0; multiplier = 1; maxMultiplier = 1; lives = 3; xp = 0; gankModeUntil = 0; shieldUntil = 0; freezeUntil = 0; ghostUntil = 0; pointerTarget = null;
  ...
  for (let i = 0; i < 2; i += 1) spawnGanker();
...
function bankScore() {
  ...
  banked += Math.round(risk * (gankModeUntil > elapsed ? 2 : 1)); risk = 0; multiplier = Math.min(10, multiplier + 1); maxMultiplier = Math.max(maxMultiplier, multiplier); xp += 20; localStorage.setItem("gankbyte-achievement-byte-snatch", "earned"); status.textContent = gankModeUntil > elapsed ? "GANK MODE payout secured." : `Score banked. Multiplier x${multiplier}.`; burst(BANK.x, BANK.y, "#c6ff3d", 30); updateHud();
}
function hitPlayer() {
...
function update(dt) {
  ...
  if (gankers.length < 2 + Math.floor(elapsed / 7) && Math.random() < dt * 1.1) spawnGanker(); if (powerups.length < 1 && Math.random() < dt * .07) spawnPowerup();
  const frozen = freezeUntil > elapsed;
  gankers.forEach((ganker) => { if (frozen) return; const direct = Math.atan2(player.y - ganker.y, player.x - ganker.x); const angle = ganker.kind === "drifter" ? direct + Math.sin(elapsed * 1.7 + ganker.phase) * .45 : direct; ganker.x += Math.cos(angle) * ganker.speed * dt; ganker.y += Math.sin(angle) * ganker.speed * dt; ganker.phase += dt; if (distance(player, ganker) < player.radius + ganker.radius) hitPlayer(); });
...
function draw() {
  ...
  ctx.save(); ctx.translate(BANK.x, BANK.y); ctx.rotate(elapsed * .5); ctx.strokeStyle = "#c6ff3d"; ctx.shadowBlur = 30; ctx.shadowColor = "#c6ff3d"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, BANK.radius, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, BANK.radius - 12, 0, Math.PI * 2); ctx.stroke(); ctx.restore(); ctx.fillStyle = "#c6ff3d"; ctx.font = "bold 11px Arial"; ctx.textAlign = "center"; ctx.fillText("BANK", BANK.x, BANK.y + 4);
  ...
  powerups.forEach((p) => { const labels = { magnet: "M", shield: "S", freeze: "F", double: "2X", ghost: "G", time: "+5" }; ctx.fillStyle = "#55e8ff"; ctx.shadowBlur = 20; ctx.shadowColor = "#55e8ff"; ctx.beginPath(); ctx.rect(p.x - 13, p.y - 13, 26, 26); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = "#0a0b0f"; ctx.font = "bold 9px Arial"; ctx.fillText(labels[p.type], p.x, p.y + 3); });
...
async function loadLeaderboard() { if (!client) { leaderboardBody.innerHTML = '<tr><td colspan="4">Global scores need the XP backend connection.</td></tr>'; return; } const result = await client.from("byte_snatch_leaderboard").select("display_name,best_score,best_multiplier").order("best_score", { ascending: false }).limit(25); if (result.error) { leaderboardBody.innerHTML = '<tr><td colspan="4">Run the Byte Snatch migration to enable scores.</td></tr>'; return; } leaderboardBody.innerHTML = result.data?.length ? result.data.map((row, i) => `<tr><td>${i + 1}</td><td>${escapeHtml(row.display_name || "GankByte Player")}</td><td>${Number(row.best_score || 0).toLocaleString()}</td><td>x${Number(row.best_multiplier || 1)}</td></tr>`).join("") : '<tr><td colspan="4">No scores yet. Be the first to bank a run.</td></tr>'; }
