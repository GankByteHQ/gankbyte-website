 (async () => {
  ...
  const cards = {
    ...
    ninja: { bestKey: "gankbyte-null-ninja-best", lastPlayedKey: "gankbyte-null-ninja-last-played", view: "null_ninja_leaderboard", table: "null_ninja_scores", label: "Null Ninja" }
    ,fighter: { bestKey: "gankbyte-stick-fighter-best", lastPlayedKey: "gankbyte-stick-fighter-last-played", view: "stick_fighter_leaderboard", table: "stick_fighter_scores", label: "Stick Fighter" }
  };
  ...
  if (gameGrid && !gameGrid.querySelector('[data-game-card="null-ninja"]')) {
    gameGrid.insertAdjacentHTML("beforeend", '<article class="live-game-card" data-game-card="null-ninja"><img class="game-thumb" src="null-ninja-thumb.svg" alt="Null Ninja digital stickman facing a corrupted network" /><div class="live-game-card-body"><span class="status-badge">Playable now</span><h3>Null Ninja</h3><p>Run through the corrupted network, strike drones, chain perfect attacks, and push into Gank Mode.</p><ul><li>Auto-run action platforming</li><li>Jump, double jump, dash, slide, and counter</li><li>Combos, Flow, Gank Mode, and power routes</li><li>Personal best, XP, and leaderboard</li></ul><p class="game-card-stat" data-game-stat="ninja">Personal best: loading</p><div class="hero-actions"><a class="button button-primary" href="null-ninja.html">Play Null Ninja <span>&nearr;</span></a><a class="text-link" href="null-ninja.html#leaderboard">View leaderboard <span>&nearr;</span></a></div></div></article>');
  }
  if (gameGrid && !gameGrid.querySelector('[data-game-card="stick-fighter"]')) {
    gameGrid.insertAdjacentHTML("beforeend", '<article class="live-game-card" data-game-card="stick-fighter"><img class="game-thumb" src="stick-fighter-thumb.svg" alt="GankByte stick fighter ready to duel in a neon arena" /><div class="live-game-card-body"><span class="status-badge">Playable now</span><h3>Stick Fighter</h3><p>Step into the neon ring, read the opponent, and chain punches, kicks, blocks, and specials before the round timer runs out.</p><ul><li>Keyboard and touch controls</li><li>Player versus CPU and local two-player modes</li><li>Round health, blocking, knockback, and specials</li><li>Personal best, XP, profile history, and leaderboard</li></ul><p class="game-card-stat" data-game-stat="fighter">Personal best: loading</p><div class="hero-actions"><a class="button button-primary" href="stick-fighter.html">Play Stick Fighter <span>&nearr;</span></a><a class="text-link" href="stick-fighter.html#leaderboard">View leaderboard <span>&nearr;</span></a></div></div></article>');
  }
  document.querySelectorAll("h2, .section-intro").forEach((node) => { node.textContent = node.textContent.replace(/Three games\.?/g, "Ten games.").replace(/Five games\.?/g, "Ten games.").replace("Byte Rush, Glitch Dash, and Symbol Catch", "Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, Byte Snatch, Signal Swarm, Packet Siege, Byte Stack, Null Ninja, and Stick Fighter").replace("Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, and Byte Snatch", "Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, Byte Snatch, Signal Swarm, Packet Siege, Byte Stack, Null Ninja, and Stick Fighter"); });
