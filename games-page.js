 (async () => {
  ...
  if (gameGrid && !gameGrid.querySelector('[data-game-card="signal-swarm"]')) {
    gameGrid.insertAdjacentHTML("beforeend", '<article class="live-game-card" data-game-card="signal-swarm"><img class="game-thumb" src="signal-swarm-thumb.svg" alt="Signal Swarm glowing creatures moving through a corrupted network" /><div class="live-game-card-body"><span class="status-badge">Playable now</span><h3>Signal Swarm</h3><p>Guide an automatic swarm through a failing network, rescue valuable Signals, and push the speed for a bigger score.</p><ul><li>60-second arcade strategy run</li><li>Three Signal types and five limited abilities</li><li>Corruption, hazards, power-ups, and combo scoring</li><li>Personal best, XP, profile history, and leaderboard</li></ul><p class="game-card-stat" data-game-stat="swarm">Personal best: loading</p><div class="hero-actions"><a class="button button-primary" href="signal-swarm.html">Play Signal Swarm <span>&nearr;</span></a><a class="text-link" href="signal-swarm.html#leaderboard">View leaderboard <span>&nearr;</span></a></div></div></article>');
  }
  ...
  document.querySelectorAll("h2, .section-intro").forEach((node) => { node.textContent = node.textContent.replace(/Three games\.?/g, "Six games.").replace(/Five games\.?/g, "Six games.").replace("Byte Rush, Glitch Dash, and Symbol Catch", "Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, Byte Snatch, and Signal Swarm").replace("Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, and Byte Snatch", "Byte Rush, Glitch Dash, Symbol Catch, Codebreaker, Byte Snatch, and Signal Swarm"); });
