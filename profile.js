...
async function loadProfile(session) {
  ...
  const [profileResult, xpResult, arenaResult, glitchResult, symbolResult, snatchResult, codebreakerResult, swarmResult, ninjaResult, fighterResult, submissionResult, dismissalResult, xpHistoryResult, challengesResult] = await Promise.all([
    ...
    profileClient.from("stick_fighter_scores").select("id,score,rounds_won,hits_landed,best_combo,perfect_blocks,specials,level_reached,created_at,status,reviewer_note,reviewed_at").eq("user_id", profileUser.id).order("created_at", { ascending: false }).limit(50),
