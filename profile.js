...
const profileSymbolBest = document.querySelector("#profile-symbol-best");
const profileSymbolRuns = document.querySelector("#profile-symbol-runs");
let profileSnatchBest = document.querySelector("#profile-snatch-best");
let profileSnatchRuns = document.querySelector("#profile-snatch-runs");
let profileCodebreakerBest = document.querySelector("#profile-codebreaker-best");
...
let profileFighterBest = document.querySelector("#profile-fighter-best");
let profileFighterRuns = document.querySelector("#profile-fighter-runs");
...
function renderProfileBadges(xp) {
  ...
  let swarmAchievements = [];
  try { swarmAchievements = JSON.parse(window.localStorage.getItem("gankbyte-signal-swarm-achievements") || "[]"); } catch { swarmAchievements = []; }
  const earned = [["First signal", xp >= 100], ["Playtester", xp >= 500], ["Builder", xp >= 1500], ["Core contributor", xp >= 3000], ["Byte Snatcher", window.localStorage.getItem("gankbyte-achievement-byte-snatch") === "earned"], ["Signal Swarm", swarmAchievements.length > 0], ["Overclocked", swarmAchievements.includes("OVERCLOCKED")]];
...
function ensureProfileSnatchCard() {
  const grid = document.querySelector(".content-grid");
  if (grid && !grid.querySelector('[data-profile-game="byte-snatch"]')) {
    grid.insertAdjacentHTML("beforeend", '<article class="content-card" data-profile-game="byte-snatch"><span class="status-badge">Byte Snatch</span><h3 id="profile-snatch-best">-</h3><p id="profile-snatch-runs">No Byte Snatch runs recorded</p></article>');
  }
  profileSnatchBest = document.querySelector("#profile-snatch-best");
  profileSnatchRuns = document.querySelector("#profile-snatch-runs");
  if (grid && !grid.querySelector('[data-profile-game="codebreaker"]')) {
    grid.insertAdjacentHTML("beforeend", '<article class="content-card" data-profile-game="codebreaker"><span class="status-badge">Codebreaker</span><h3 id="profile-codebreaker-best">-</h3><p id="profile-codebreaker-runs">No Codebreaker runs recorded</p></article>');
  }
  profileCodebreakerBest = document.querySelector("#profile-codebreaker-best");
  profileCodebreakerRuns = document.querySelector("#profile-codebreaker-runs");
  if (grid && !grid.querySelector('[data-profile-game="signal-swarm"]')) {
    grid.insertAdjacentHTML("beforeend", '<article class="content-card" data-profile-game="signal-swarm"><span class="status-badge">Signal Swarm</span><h3 id="profile-swarm-best">-</h3><p id="profile-swarm-runs">No Signal Swarm runs recorded</p></article>');
  }
  ...
  if (grid && !grid.querySelector('[data-profile-game="stick-fighter"]')) {
    grid.insertAdjacentHTML("beforeend", '<article class="content-card" data-profile-game="stick-fighter"><span class="status-badge">Stick Fighter</span><h3 id="profile-fighter-best">-</h3><p id="profile-fighter-runs">No Stick Fighter matches recorded</p></article>');
  }
  profileFighterBest = document.querySelector("#profile-fighter-best");
  profileFighterRuns = document.querySelector("#profile-fighter-runs");
}

function renderHistory(arenaRows, glitchRows, symbolRows, snatchRows, codebreakerRows, swarmRows, ninjaRows, fighterRows) {
  const rows = [...(arenaRows || []).map((row) => ({ game: "Byte Rush", score: row.score, result: `Wave ${row.wave}`, seconds: row.run_seconds, created_at: row.created_at })), ...(glitchRows || []).map((row) => ({ game: "Glitch Dash", score: row.score, result: `Streak ${row.streak}`, seconds: row.run_seconds, created_at: row.created_at })), ...(symbolRows || []).map((row) => ({ game: "Symbol Catch", score: row.score, result: `Streak ${row.best_streak}`, seconds: row.run_seconds, created_at: row.created_at })), ...(snatchRows || []).map((row) => ({ game: "Byte Snatch", score: row.score, result: `x${row.best_multiplier} multiplier`, seconds: row.run_seconds, created_at: row.created_at })), ...(codebreakerRows || []).map((row) => ({ game: "Codebreaker", score: row.score, result: `Level ${String(row.level || 0).padStart(2, "0")}`, seconds: row.run_seconds, created_at: row.created_at })), ...(swarmRows || []).map((row) => ({ game: "Signal Swarm", score: row.score, result: `${row.signals_saved} saved // x${row.best_combo}`, seconds: row.run_seconds, created_at: row.created_at })), ...(ninjaRows || []).map((row) => ({ game: "Null Ninja", score: row.score, result: `${row.distance}m // ${row.kills} kills`, seconds: row.run_seconds, created_at: row.created_at })), ...(fighterRows || []).map((row) => ({ game: "Stick Fighter", score: row.score, result: `Level ${row.level_reached || 1} // ${row.levels_cleared || 0} levels // ${row.hits_landed} hits`, seconds: 0, created_at: row.created_at }))].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 20);
...
function renderRejections(submissionRows, arenaRows, glitchRows, symbolRows, dismissalRows, snatchRows, codebreakerRows, swarmRows, ninjaRows, fighterRows) {
  const records = [
    ...
    ...(fighterRows || []).map((row) => ({ key: profileNoticeKey("stick-fighter", row.id), group: "score:Stick Fighter", type: "stick-fighter", id: row.id, label: "Stick Fighter score", status: row.status, reason: row.reviewer_note, created_at: row.created_at, reviewed_at: row.reviewed_at }))
...
async function loadProfile(session) {
  ...
  const [profileResult, xpResult, arenaResult, glitchResult, symbolResult, snatchResult, codebreakerResult, swarmResult, ninjaResult, fighterResult, submissionResult, dismissalResult, xpHistoryResult, challengesResult] = await Promise.all([
    ...
    profileClient.from("symbol_catch_scores").select("id,score,best_streak,mode,run_seconds,created_at,status,reviewer_note,reviewed_at").eq("user_id", profileUser.id).order("created_at", { ascending: false }).limit(50),
    ...
    profileClient.from("stick_fighter_scores").select("id,score,levels_cleared,hits_landed,best_combo,perfect_blocks,specials,level_reached,created_at,status,reviewer_note,reviewed_at").eq("user_id", profileUser.id).order("created_at", { ascending: false }).limit(50),
  ...
  const dataError = [profileResult, xpResult, arenaResult, glitchResult, symbolResult, snatchResult, codebreakerResult, swarmResult, ninjaResult, fighterResult, submissionResult, dismissalResult, xpHistoryResult, challengesResult].find((result) => result?.error);
  ...
  const fighterRows = fighterResult.data || [];
  ...
  const approvedFighterRows = fighterRows.filter((row) => row.status !== "rejected");
  ...
  const fighterBest = approvedFighterRows.reduce((best, row) => Math.max(best, Number(row.score || 0)), 0);
  ...
  if (profileSymbolBest) profileSymbolBest.textContent = symbolBest ? symbolBest.toLocaleString() : "-";
  if (profileSymbolRuns) profileSymbolRuns.textContent = `${approvedSymbolRows.length} submitted run${approvedSymbolRows.length === 1 ? "" : "s"}`;
  if (profileSnatchBest) profileSnatchBest.textContent = snatchBest ? snatchBest.toLocaleString() : "-";
  if (profileSnatchRuns) profileSnatchRuns.textContent = `${approvedSnatchRows.length} submitted run${approvedSnatchRows.length === 1 ? "" : "s"}`;
  if (profileCodebreakerBest) profileCodebreakerBest.textContent = codebreakerBest ? codebreakerBest.toLocaleString() : "-";
  ...
  if (profileFighterBest) profileFighterBest.textContent = fighterBest ? fighterBest.toLocaleString() : "-";
  if (profileFighterRuns) profileFighterRuns.textContent = `${approvedFighterRows.length} submitted match${approvedFighterRows.length === 1 ? "" : "es"}`;
  renderHistory(approvedArenaRows, approvedGlitchRows, approvedSymbolRows, approvedSnatchRows, approvedCodebreakerRows, approvedSwarmRows, approvedNinjaRows, approvedFighterRows);
  ...
  renderRejections(submissionResult.data || [], arenaRows, glitchRows, symbolRows, dismissalRows, snatchRows, codebreakerRows, swarmRows, ninjaRows, fighterRows);
