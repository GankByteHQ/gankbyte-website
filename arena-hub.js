(() => {
  ...
  async function load() {
    ...
    const [byte, glitch, symbol, snatch, xp] = await Promise.all([
    ...
    if (byte.error || glitch.error || symbol.error || snatch.error || (user && xp.error)) {
    ...
    const [arena, dash, symbolRuns, snatchRuns] = await Promise.all([
