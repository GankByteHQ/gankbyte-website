(() => {
  ...
  function update(dt) {
    if (!running || paused || ended) return; matchTime = Math.max(0, matchTime - dt); const pLeft = keys.has("a"); const pRight = keys.has("d"); if (pLeft) move(player, -1); else if (pRight) move(player, 1); else if (!player.attacking) player.vx *= .86; block(player, keys.has("l"));
  ...
  function drawStick(f, color, accent) {
    const x = f.x, y = f.y, glow = f.invulnerable > 0 ? "#ffffff" : color; ctx.save(); ctx.strokeStyle = glow; ctx.fillStyle = "#090c12"; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.shadowColor = glow; ctx.shadowBlur = f.hurt > 0 ? 18 : 8; if (f.hurt > 0) ctx.translate(rand(-2, 2), 0);
    ...
    ctx.strokeStyle = accent; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(x - 9, y + 11); ctx.lineTo(x - 4, y + 11); ctx.moveTo(x + 4, y + 11); ctx.lineTo(x + 9, y + 11); ctx.stroke(); if (f.attacking && f.attack) { ctx.strokeStyle = f.attack.kind === "special" ? "#c6ff3d" : accent; ctx.lineWidth = f.attack.kind === "special" ? 8 : 5; ctx.shadowColor = ctx.strokeStyle; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(x + f.facing * 30, y + 40, f.attack.reach * .45, f.facing > 0 ? -.9 : 2.2, f.facing > 0 ? .9 : 4); ctx.stroke(); } if (f.blocking) { ctx.strokeStyle = "#55e8ff"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(x + f.facing * 16, y + 42, 29, f.facing > 0 ? -1.2 : 1.9, f.facing > 0 ? 1.2 : 4.3); ctx.stroke(); } ctx.restore();
