...
Supabase provides `SUPABASE_URL` and `SUPABASE_SECRET_KEYS` automatically. The
function reads the default server-side secret from that protected dictionary;
never put it in the website or commit it to GitHub. The sending domain/address
must be verified with the email provider before production use.
